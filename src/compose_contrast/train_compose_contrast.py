...
def evaluate(it_net, encoder, val_dl, criterion, config):
    val_loss = 0
    step = 0
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            ...
            T = create_random_transform(pointcloud.shape[0], max_rotation_deg=config.max_rotation_deg, max_translation=config.max_translation, dtype=pointcloud.dtype).to(config.device)

            pc = apply_tfm(pointcloud, T)
            pc, _, _ = it_net(pc)
            pc = pc.transpose(2, 1)

            pc1 = apply_corruptions(pc).transpose(2, 1)
            pc2 = apply_corruptions(pc).transpose(2, 1)

            pc1_feat, _, _ = encoder(pc1)
            pc2_feat, _, _ = encoder(pc2)

            val_loss += criterion(pc1_feat, pc2_feat).item()
            step += 1
...
def train(config):
    ...
    if config.load == "True":
        ...
        lr = checkpoint["lr"]
        optimizer = torch.optim.AdamW(encoder.parameters(), lr=lr, weight_decay=config.wd)
    ...
    while step < config.num_steps:
        ...
        for batch in pbar:
            ...
            loss = criterion(pc1_feat, pc2_feat)
            ...
            if step % config.eval_every == 0:
                ...
                val_loss = evaluate(it_net, encoder, val_dl, criterion, config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="results/compose_contrast")
