...
with open("/Users/meharsingh/Documents/rigid-alignment-siamese-network/src/ras_pointnet/chair_0892.off", 'r') as f:
    P, _ = read_off(f)
P = np.array(P)
P.shape
...
print(is_rigid(T_3[0][:3,:3].detach().numpy()))
print("==============")

P_trans_0_numpy = P_trans[0].detach().numpy()
# P_trans_0_numpy = (P[0] @ T_3[0][:3,:3]).detach().numpy()
# print(P_trans_0_numpy.shape)
